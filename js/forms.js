...
const FormController = {
    ...
    initInputs() {
        ...
        inputCep?.addEventListener("blur", async () => {
            ...
            if (cep.length === 8) {
                ...
                if (dados && !dados.erro) {
                    document.querySelector("#endereco").value = `${dados.logradouro}${dados.bairro ? ', ' + dados.bairro : ''}`;
        ...
        selectTipo?.addEventListener("change", () => {
            ...
            inputReg.placeholder = tipo === "PF" ? "000.000.000-00" : "00.000.000/0000-00";
    ...
    handleSave(e) {
        ...
        const duplicado = EmpreendimentoStorage.buscarTodos().find(emp => 
            emp.registro === dados.registro && emp.id !== Number(id)
        );
        
        if (duplicado) {
            return alert(`ERRO DE NEGÓCIO: O registro ${dados.registro} já está vinculado a "${duplicado.nome}".`);
        }

        id ? EmpreendimentoStorage.atualizar(id, dados) : EmpreendimentoStorage.adicionar(dados);
    ...
    prepararEdicao(id) {
        ...
        document.querySelector("#titulo-modal-form").textContent = `Edição: ${emp.nome}`;
        ...
        const labelReg = document.querySelector("#label-registro");
        if (labelReg) labelReg.textContent = emp.tipoPessoa === "PF" ? "CPF" : "CNPJ";
